...
mod tests {
    ...
    fn test_original_bug_reproduction_typescript() {
        ...
        assert!(
            ts_definition.contains("string_to_vec_i64: Partial<Record<string, Array<number>>>;")
        );
        assert!(
            ts_definition.contains("string_to_vec_f64: Partial<Record<string, Array<number>>>;")
        );
        assert!(
            ts_definition.contains("string_to_vec_bool: Partial<Record<string, Array<boolean>>>;")
        );
        assert!(
        ...
        assert!(
            zod_schema
                .contains("string_to_vec_i64: z.record(z.string(), z.array(z.number().int()))")
        );
        assert!(
            zod_schema.contains("string_to_vec_f64: z.record(z.string(), z.array(z.number()))")
        );
        assert!(
            zod_schema.contains("string_to_vec_bool: z.record(z.string(), z.array(z.boolean()))")
        );
        assert!(
            zod_schema.contains("string_to_vec_string: z.record(z.string(), z.array(z.string()))")
        );
    }
